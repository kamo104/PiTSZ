...
def solution(args):
    ...
    # Sort tasks by (deadline - processing time) and keep track of their original indices
    indexed_tasks = [(idx, task) for idx, task in enumerate(instance.tasks)]
    sorted_tasks = sorted(indexed_tasks, key=lambda t: t[1]['dj'] - min(t[1]['pkj']))

    # Divide tasks equally among the 5 workers
    # num_workers = 5
    # tasks_per_worker = [[] for _ in range(num_workers)]
    # for i, (task_idx, _) in enumerate(sorted_tasks):
    #     tasks_per_worker[i % num_workers].append(task_idx + 1)  # Task indices are 1-based
    # solution.jobs = tasks_per_worker
    ...
    while time.time() - start_time < args.time:
        ...
        for task_idx, _ in sorted_tasks:
            random_worker = random.randint(0, num_workers - 1)
            tasks_per_worker[random_worker].append(task_idx + 1)

        # Update the solution and calculate the score
        solution.jobs = tasks_per_worker
        current_score = get_score(instance, solution)

        # If this random assignment is better, update the best solution
        if current_score < best_score:
            best_score = current_score
            best_jobs = [worker_tasks[:] for worker_tasks in tasks_per_worker]
        print(best_score)


    # Assign the best solution found
    solution.jobs = best_jobs
